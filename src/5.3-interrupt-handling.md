...
In STREAMS (as we saw in [STREAMS Framework](./4.10-streams-framework.md)), the bottom half is often the **service procedure**, invoked via `runqueues()` when the queue is enabled. This separation ensures that the top half remains brief, while the bottom half can safely block, allocate memory, and interact with user processes.
